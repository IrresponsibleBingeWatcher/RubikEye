...
class RubikSolver:
    ...
    def solve(self):
        ...
        for face_color_name in solve_order:
            ...
            for i in range(3):
                row_evaluated = []
                # FIX: Reverse the column order to account for horizontal flip
                for j in range(2, -1, -1):  # Changed from range(3) to range(2, -1, -1)
                ...
                # Display the row in the order we read it (reversed)
